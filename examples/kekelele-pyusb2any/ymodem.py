...
class YMODEM(object):
    ...
    send entry
    '''
    def send(self, file_stream, file_name, file_size,retry=20, timeout=15, callback=None):
        try:
            packet_size = dict(
                ymodem = 1024,
                ymodem128 = 128
        ...
        filename = file_name.encode(encoding="utf-8") + self.header_pad
        #filesize = (hex(len(file_stream.read()))[2:]).encode(encoding="utf-8") + self.header_pad
        filesize = (hex(file_size)[2:]).encode(encoding="utf-8") + self.header_pad
        data = filename + filesize + (128 - len(filename+filesize))*self.header_pad
        ...
        while True:
            ...
            data = data + (packet_size - len(data))*self.pad
            checksum = self._make_send_checksum(data)
    ...
    recv entry
    '''
    def recv(self, retry=20, timeout=15, delay=0.01):
        error_count = 0 
        cancel = 0
        char = ""
        file_stream = bytes()
        ...
        while True:
            while True:
                if char == SOH:
                    ...
                    self.putc(ACK)
                    self.log.info("Success Received")
                    self.log.info("fileSize: %d Name:%s length:%d"%(file_size, file_name, len(file_stream)))
                    return file_stream, file_name, file_size
                elif char == CAN:
                    if cancel:
                        return None
                    else:
            ...
            if not (seq == seq_oc == sequence):
                ...
                valid, data = self._verify_recv_checksum(data)

                if valid:
                    if seq == 0 and receive_flag == False:
                        receive_flag = True
                        file_name, file_size =self._get_file_properity(data)
                        self.log.info("Name:"+file_name + " Size:" + str(file_size))
                        self.putc(ACK)
                        self.log.info("ACK >>>")
                        
                        #self.putc(CRC)
                        #self.log.info("CRC >>>")
                        totalsequence = file_size%packet_size
                        if totalsequence>0:
                            totalsequence = 1
                        else:
                            totalsequence = 0
                        totalsequence = file_size//packet_size + totalsequence
                        sequence = (sequence + 1) % 0x100
                        char = self.getc(1)
                        continue
                    else:
                        file_stream = file_stream + data
                        self.putc(ACK)
                        #self.log.info("ACK >>>")
                        seqnum = seqnum +1
                        sequence = (sequence + 1) % 0x100
                        self.log.info("Total: {:.0%}".format(seqnum/totalsequence))
                        char = self.getc(1)
                        continue
                else:
                    self.log.info("CRCnotOK")
    ...
    # Header byte
    # example:SOH 00 FF foo.c 3232 NUL[118] CRCH CRCL
    ...
    def _verify_recv_checksum(self, data):
        ...
        return valid, data
    def _get_file_properity(self, data):
        namelist= data.decode().split(self.header_pad.decode())
        filename = namelist[0]
        filesize = int('0x'+namelist[1],16)
